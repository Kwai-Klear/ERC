...
def compute_score(
    ...
    if not is_use_math_verify:
        ...
        #     except TimeoutError:
        #         reward = scoring_config.incorrect_score

        math_verify_exec_path = "/mmu_nlp_hdd/panleiyu/verl_0929/verl/utils/reward_score/math_verify_impl.py"
        ...
        command.insert(4, f"--whitelist={math_verify_exec_path}")
        command.extend(["/share/miniconda3/envs/verl_szp_py310/bin/python", math_verify_exec_path])

        # input_str = f"{solution_str} {ground_truth}"
        input_str = (solution_str, ground_truth)
